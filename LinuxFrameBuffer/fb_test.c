...
#include <string.h> //For last error
#include <errno.h>
...
static int get_framebuffer(struct simple_fb *fb, unsigned bpp)
{
    ...
    printf("bpp = %d , vi.bits_per_pixel = %d\n",bpp,vi.bits_per_pixel);
    ...
    printf("user space frame buffer = 0x%X\n", bits );
    ...
    fb->data = bits; /*[CLS]: frame buffer address, 直接寫*/
...
{
    #define DUMPVAR( _var_ )\
     fprintf(stderr, "vi."#_var_" = %d\n", (vi)->_var_ );

    #define DUMPVAR_FI( _var_ )\
     fprintf(stderr, "fi."#_var_" = %d\n", (fi)->_var_ );
    ...
    DUMPVAR( pixclock );
    DUMPVAR( left_margin );
    DUMPVAR( right_margin );
    DUMPVAR( upper_margin);		/* time from sync to picture	*/
	DUMPVAR( lower_margin);
	DUMPVAR( hsync_len);		/* length of horizontal sync	*/
	DUMPVAR( vsync_len);		/* length of vertical sync	*/
	DUMPVAR( sync);			/* see FB_SYNC_*ex.FB_SYNC_HOR_HIGH_ACT			*/
	DUMPVAR( vmode);			/* see FB_VMODE_*ex.FB_VMODE_NONINTERLACED 		*/
	DUMPVAR( rotate);	



    fprintf(stderr, "fi.id = %s\n", fi->id );	/* identification string eg "TT Builtin" */
	DUMPVAR_FI( smem_start);	/* Start of frame buffer mem */
					/* (physical address) */
	DUMPVAR_FI( smem_len);			/* Length of frame buffer mem */
	DUMPVAR_FI( type);			/* see FB_TYPE_*ex.FB_TYPE_PACKED_PIXELS			*/
	DUMPVAR_FI( type_aux);			/* Interleave for interleaved Planes */
	DUMPVAR_FI( visual);			/* see FB_VISUAL_*ex.FB_VISUAL_MONO01			*/ 
	DUMPVAR_FI( xpanstep);			/* zero if no hardware panning  */
	DUMPVAR_FI( ypanstep);			/* zero if no hardware panning  */
	DUMPVAR_FI( ywrapstep);		/* zero if no hardware ywrap    */
	DUMPVAR_FI( line_length);		/* length of a line in bytes    */
	DUMPVAR_FI( mmio_start);	/* Start of Memory Mapped I/O   */
					/* (physical address) */
	DUMPVAR_FI( mmio_len);			/* Length of Memory Mapped I/O  */
	DUMPVAR_FI( accel);			/* Indicate to driver which	*/
...
int gr_init(int bpp, int id)
{
    ...
    if (!access("/dev/tty0", F_OK)) {  /*[CLS] : should be fail, only /dev/tty and /dev/ttyGS0-3 and /dev/ttySC045*/
        printf("/dev/tty0 access ok\n");
        ...
        printf("dev/tty0 open OK\n");
        if(ioctl(fd, KDSETMODE, (void*) KD_GRAPHICS)) {
            printf("dev\\tty0 ioctl failed\n");
            close(fd);
            return -1;
        }
        printf("\\dev\\tty0 ioctl ok\n");
